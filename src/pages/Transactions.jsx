...
import logo from "../assets/chat_gpt.png"; // ✅ your logo
...
function Transactions() {
  ...
  // 🔄 Fetch Footer (Admin Panel)
  const fetchFooter = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/settings");
      if (res.data?.footerText) {
        setFooter(res.data.footerText);
      }
    } catch {
      console.warn("Footer API not available, using default");
    }
  };
  ...
  const generatePDF = () => {
    ...
    img.onload = () => {
      for (let i = 0; i < data.length; i += rowsPerPage) {
        ...
        // 🎨 PREMIUM TABLE
        //doc.setFont("helvetica"); // default limited
        autoTable(doc, {
          startY: 30,
